...
export default function IncomePage(){
  ...
  const updRec=(id:string,patch:Partial<RecurringItem>)=>save({...plan,income:ri.map(x=>x.id===id?{...x,...patch}:x)});
  ...
  return(
    <div className='space-y-5'>
      <div><h1 className='text-2xl font-semibold text-slate-900 dark:text-slate-100'>Income</h1><p className='text-sm text-slate-500'>Recurring and one-time income.</p></div>
      <div className='grid gap-3 sm:grid-cols-3'>
        ...
        <div className='rounded-2xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900'><div className='text-xs uppercase tracking-wide text-slate-500'>Recurring</div><div className='mt-1 text-xl font-semibold text-emerald-600'>{money(recTotal,cur)}</div></div>
        <div className='rounded-2xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900'><div className='text-xs uppercase tracking-wide text-slate-500'>One-time</div><div className='mt-1 text-xl font-semibold text-emerald-600'>{money(otTotal,cur)}</div></div>
      </div>
      <div className='rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900'>
        <div className='mb-3 flex flex-wrap gap-2'>
          <button className='rounded-xl bg-blue-600 px-3 py-2 text-sm font-medium text-white' onClick={()=>save({...plan,income:[...ri,newRecurringItem('income')]})}>+ Recurring</button>
          <button className='rounded-xl border border-slate-200 px-3 py-2 text-sm dark:border-slate-700' onClick={()=>save({...plan,oneTimeIncome:[...oi,newOneTimeItem('income',editMonth)]})}>+ One-time</button>
        </div>
        <div className='space-y-3'>
          {ri.map(item=>{const ma=amountForMonth(item,editMonth);return(<div key={item.id} className='rounded-xl border border-slate-200 p-3 dark:border-slate-800'><div className='grid gap-2 md:grid-cols-6'>
            ...
            <button className='rounded-xl border border-rose-200 px-2 py-2 text-sm text-rose-600' onClick={()=>save({...plan,income:ri.filter(x=>x.id!==item.id)})}>Remove</button>
          </div></div>);})}
          {!ri.length&&<div className='text-sm text-slate-500'>No recurring income yet.</div>}
        </div>
      </div>
      <div className='rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900'>
        <div className='text-sm font-medium text-slate-900 dark:text-slate-100 mb-3'>One-time income</div>
        <div className='space-y-2'>
          {oi.map(item=>(<div key={item.id} className='grid gap-2 md:grid-cols-4'>
            <input className={inp} value={item.name} onChange={e=>save({...plan,oneTimeIncome:oi.map(x=>x.id===item.id?{...x,name:e.target.value}:x)})}/>
            <select className={inp} value={item.monthISO} onChange={e=>save({...plan,oneTimeIncome:oi.map(x=>x.id===item.id?{...x,monthISO:e.target.value}:x)})}>{monthOptions.map(m=><option key={m} value={m}>{m}</option>)}</select>
            <input className={inp} type='text' defaultValue={money(item.amount,cur)} key={'ot'+item.id+item.amount} onBlur={e=>save({...plan,oneTimeIncome:oi.map(x=>x.id===item.id?{...x,amount:asNum(e.target.value)}:x)})}/>
            <button className='rounded-xl border border-rose-200 px-2 py-2 text-sm text-rose-600' onClick={()=>save({...plan,oneTimeIncome:oi.filter(x=>x.id!==item.id)})}>Remove</button>
          </div>))}
          {!oi.length&&<div className='text-sm text-slate-500'>No one-time income yet.</div>}
        </div>
      </div>
    </div>
  );
