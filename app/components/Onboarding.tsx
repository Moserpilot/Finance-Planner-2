...
export function Onboarding({ onComplete }: { onComplete: () => void }) {
  ...
  function finish() {
    ...
    const retNum = Number(ret.replace(/[^0-9.]/g,'')) || 5;
    const plan = {
      ...base,
      currency,
    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4'>
      <div className='w-full max-w-sm rounded-3xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-950 overflow-hidden'>
        ...
        {step===1 && <div className='flex flex-col px-6 py-8 gap-5'>
          ...
          <div className='text-sm text-slate-500 dark:text-slate-400 -mt-3'>Set your currency and planning start date.</div>
          ...
          <div><div className={lbl}>Planning start month</div>
            <input type='month' className={inp} value={startMonth} onChange={e=>setStartMonth(e.target.value)} />
          </div>
        ...
        {step===2 && <div className='flex flex-col px-6 py-8 gap-5'>
          ...
          <div className='text-sm text-slate-500 dark:text-slate-400 -mt-3'>Set a net worth target and expected return.</div>
          <div><div className={lbl}>Net worth goal</div>
            <div className={row}>
              ...
              <input type='text' className={rin+' pr-3'} placeholder='1,000,000' value={goal} onChange={e=>setGoal(e.target.value)} />
            </div>
          </div>
          <div><div className={lbl}>Expected annual return</div>
            <div className={row}>
              <input type='text' style={{width:`calc(${(ret||'7').length}ch + 0.75rem)`}} className='bg-transparent pl-3 py-3 text-sm text-slate-900 dark:text-slate-100 outline-none min-w-0' placeholder='7' value={ret} onChange={e=>setRet(e.target.value)} />
              <span className='pr-3 text-sm text-slate-400 select-none'>%</span>
            </div>
        ...
        {step===3 && <div className='flex flex-col px-6 py-8 gap-5'>
          ...
          <div className='text-sm text-slate-500 dark:text-slate-400 -mt-3'>Add a bank or investment account to start tracking.</div>
