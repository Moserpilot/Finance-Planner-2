...
function markOnboardingDone(): void {
  if (typeof window !== 'undefined') localStorage.setItem(ONBOARDING_KEY, 'done');
}
...
export function Onboarding({ onComplete }: Props) {
  ...
  function finish() {
    const base: Plan = createDefaultPlan();
        ...
        {step === 2 && (
          <div className='flex flex-col px-6 py-8 gap-5'>
            <div>
              ...
              <div className='text-sm text-slate-500 dark:text-slate-400'>Set a net worth target and expected return.</div>
            </div>
            <div><div className={lbl}>Net worth goal ({currency})</div>
              <input type='text' className={inp} placeholder='e.g. 1000000' value={goal} onChange={e => setGoal(e.target.value)} />
            </div>
            <div><div className={lbl}>Expected annual return</div>
              <div className='flex items-center rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 focus-within:border-blue-400'>
                <input type='text' placeholder='7' className='w-full bg-transparent pl-3 py-3 text-sm text-slate-900 dark:text-slate-100 outline-none' value={ret} onChange={e => setRet(e.target.value)} />
        ...
        {step === 3 && (
          <div className='flex flex-col px-6 py-8 gap-5'>
            <div>
              ...
              <div className='text-sm text-slate-500 dark:text-slate-400'>Add a bank or investment account to start tracking.</div>
            </div>
