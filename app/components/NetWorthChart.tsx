...
export function NetWorthChart({currency,series,startMonthISO,heightPx=500,goalNetWorth=0}:{currency:string;series:SeriesPoint[];startMonthISO:string;heightPx?:number;goalNetWorth?:number;}){
  ...
  const chart=useMemo(()=>{
    ...
    const evPx=Math.max(1,Math.ceil(80/Math.max(1,pxPer)));
    const evM=lastM<=12?2:lastM<=24?3:lastM<=60?6:12;
    ...
    // Goal line
    const goalY=(goalNetWorth>0&&goalNetWorth>=tickMin&&goalNetWorth<=tickMax)?toY(goalNetWorth):null;
    return{VW,pts,lineD,areaD,xTicks,yTicks,x0,x1,goalY,tickMin,tickMax};
    ...
    <div className='w-full' style={{height:heightPx}}>
      ...
      <div ref={containerRef} className='w-full rounded-2xl border border-slate-200/60 dark:border-slate-800/60' style={{height:heightPx-24}}>
        <svg width={chart.VW} height={VH} className='block text-blue-500 dark:text-blue-400' onPointerMove={onPointer} onPointerDown={onPointer} onPointerLeave={()=>setHoverPct(null)} style={{touchAction:'none',width:'100%',height:'100%'}}>
          ...
          {chart.xTicks.map((t,i)=>(<g key={i}><text x={t.x} y={VH-pad.b+16} textAnchor={i===0?'start':i===chart.xTicks.length-1?'end':'middle'} fontSize='16' fill={ax} className='text-slate-900 dark:text-slate-100' opacity={0.85}>{lbl(startMonthISO,t.m)}</text></g>))}
          ...
          {chart.goalY!==null&&(
            <g>
              ...
              <text x={chart.VW-pad.r+4} y={chart.goalY+4} fontSize='11' fill='#f59e0b' opacity={0.9} textAnchor='start'>Goal</text>
            </g>
          )}
        </svg>
      </div>
