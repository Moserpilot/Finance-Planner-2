...
export function NetWorthChart({currency,series,startMonthISO,heightPx=500}:{currency:string;series:SeriesPoint[];startMonthISO:string;heightPx?:number;}){
  ...
  const ax='#94a3b8',axd='#94a3b8';
  function onPointer(e:React.PointerEvent<SVGSVGElement>){const svg=e.currentTarget;const rect=svg.getBoundingClientRect();const xPx=e.clientX-rect.left;const xSvg=xPx/rect.width*VW;const clamped=Math.min(chart.x1,Math.max(chart.x0||pad.l,xSvg));setHoverPct((clamped-(chart.x0||pad.l))/((chart.x1-(chart.x0||pad.l))||1));}
  if(!mounted)return<div className='w-full rounded-2xl border border-slate-200/60 dark:border-slate-800/60 bg-slate-50 dark:bg-slate-900/40' style={{height:heightPx}}/>;
  return(
    <div className='w-full' style={{height:heightPx}}>
      ...
      <div className='w-full' style={{height:heightPx-24}}>
        <svg viewBox={'0 0 '+VW+' '+VH} width='100%' height='100%' preserveAspectRatio='xMidYMid meet' className='block text-blue-500 dark:text-blue-400' onPointerMove={onPointer} onPointerDown={onPointer} onPointerLeave={()=>setHoverPct(null)} style={{touchAction:'none'}}>
          ...
          <path d={chart.lineD} fill='none' stroke='currentColor' opacity={0.4} strokeWidth={10} strokeLinecap='round' filter='url(#glow)'/>
          <path d={chart.lineD} fill='none' stroke='currentColor' opacity={0.95} strokeWidth={3} strokeLinecap='round'/>
          {lastPt&&<g><circle cx={lastPt.x} cy={lastPt.y} r={5} fill='currentColor' opacity={0.95}/><text x={lastPt.x-10} y={lastPt.y-12} textAnchor='end' fontSize='13' fill={ax} opacity={0.9}>{fmtK(lastPt.v,currency)}</text></g>}
          {hover&&<g><line x1={hover.x} x2={hover.x} y1={pad.t} y2={VH-pad.b} stroke='currentColor' opacity={0.2} strokeDasharray='5 4'/><circle cx={hover.x} cy={hover.y} r={6} fill='currentColor' opacity={0.95}/></g>}
        </svg>
      </div>
    </div>
  );
