...
export default function ExpensesPage() {
  ...
  const updRec = (id: string, patch: Partial<RecurringItem>) =>
    save({ ...plan, expenses: re.map(x => x.id === id ? { ...x, ...patch } : x) });
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">Recurring and one-time expenses.</p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="mt-1 text-xl font-semibold text-rose-600 dark:text-rose-400">{money(recTotal, cur)}</div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
          <div className="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">One-time</div>
          <div className="mt-1 text-xl font-semibold text-rose-600 dark:text-rose-400">{money(otTotal, cur)}</div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <button
            ...
            onClick={() => save({ ...plan, expenses: [...re, newRecurringItem('expense')] })}>
            + Recurring
          </button>
        </div>

        {re.length > 0 && (
          <div className="mb-1 hidden grid-cols-7 gap-2 px-1 md:grid">
            <div className="text-xs font-medium text-slate-400 dark:text-slate-500">Name</div>
            <div className="text-xs font-medium text-slate-400 dark:text-slate-500">Category</div>
        ...
        <div className="space-y-2">
              ...
              <div key={item.id} className="rounded-xl border border-slate-100 p-3 dark:border-slate-800">
                <div className="grid gap-2 md:grid-cols-7">
                  ...
                  <select className={inp} value={item.category || ''}
                    onChange={e => updRec(item.id, { category: (e.target.value as ExpenseCategory) || undefined })}>
                  ...
                  <button
                    ...
                    onClick={() => save({ ...plan, expenses: re.filter(x => x.id !== item.id) })}>
                    Remove
                  </button>
                </div>
              </div>
          ...
          {!re.length && (
            <div className="rounded-xl border border-dashed border-slate-200 p-6 text-center dark:border-slate-700">
              <div className="text-sm font-medium text-slate-500 dark:text-slate-400">No recurring expenses yet</div>
              <div className="mt-1 text-xs text-slate-400 dark:text-slate-500">Click <span className="font-semibold">+ Recurring</span> above to add rent, utilities, subscriptions, or any regular expense.</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="space-y-2">
          {oe.map(item => (
            <div key={item.id} className="grid gap-2 md:grid-cols-5">
              <input className={inp} value={item.name} placeholder="Description"
                onChange={e => save({ ...plan, oneTimeExpenses: oe.map(x => x.id === item.id ? { ...x, name: e.target.value } : x) })} />
              <select className={inp} value={item.category || ''}
                onChange={e => save({ ...plan, oneTimeExpenses: oe.map(x => x.id === item.id ? { ...x, category: (e.target.value as ExpenseCategory) || undefined } : x) })}>
                <option value="">Category</option>
                {EXPENSE_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
              <select className={inp} value={item.monthISO}
                onChange={e => save({ ...plan, oneTimeExpenses: oe.map(x => x.id === item.id ? { ...x, monthISO: e.target.value } : x) })}>
                {monthOptions.map(m => <option key={m} value={m}>{monthLabel(m)}</option>)}
              </select>
              <input className={inp} type="text"
                defaultValue={money(item.amount, cur)}
                key={'ot' + item.id + item.amount}
                onBlur={e => save({ ...plan, oneTimeExpenses: oe.map(x => x.id === item.id ? { ...x, amount: asNum(e.target.value) } : x) })} />
              <button
                className="mt-1 rounded-xl border border-rose-200 px-2 py-2 text-sm font-medium text-rose-600 hover:bg-rose-50 dark:border-rose-900/40 dark:text-rose-400 dark:hover:bg-rose-500/10"
                onClick={() => save({ ...plan, oneTimeExpenses: oe.filter(x => x.id !== item.id) })}>
                Remove
              </button>
            </div>
          ))}
          {!oe.length && (
            <div className="rounded-xl border border-dashed border-slate-200 p-6 text-center dark:border-slate-700">
              <div className="text-sm font-medium text-slate-500 dark:text-slate-400">No one-time expenses yet</div>
              <div className="mt-1 text-xs text-slate-400 dark:text-slate-500">Add car repairs, medical bills, vacations, or any non-recurring cost.</div>
            </div>
          )}
        </div>
      </div>
