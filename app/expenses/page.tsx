...
export default function ExpensesPage() {
  ...
  // Debounced save (smoother typing)
  useEffect(() => {
  ...
  const monthTotal = useMemo(() => {
    return plan.expenses.reduce((sum, it) => sum + amountForMonth(it, editMonthISO), 0);
  }, [plan.expenses, editMonthISO]);
  ...
  function updateItem(id: string, patch: Partial<RecurringItem>) {
    setPlan({
      ...plan,
      expenses: plan.expenses.map((it) => (it.id === id ? { ...it, ...patch } : it)),
    });
  }
  ...
  function addRecurring() {
    setPlan({ ...plan, expenses: [...plan.expenses, newRecurringItem('expense')] });
  }

  function updateOneTime(id: string, patch: any) {
    setPlan({
      ...plan,
      oneTimeExpenses: plan.oneTimeExpenses.map((it) => (it.id === id ? { ...it, ...patch } : it)),
    });
  }

  function removeOneTime(id: string) {
    setPlan({ ...plan, oneTimeExpenses: plan.oneTimeExpenses.filter((it) => it.id !== id) });
  }

  function addOneTime() {
    setPlan({
      ...plan,
      oneTimeExpenses: [...plan.oneTimeExpenses, newOneTimeItem('expense', editMonthISO)],
    });
  }
  ...
  return (
    <div className="mx-auto max-w-6xl p-6">
      <div className="mb-6 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Add recurring and one-time expenses. Use the month selector to edit month-specific amounts.
          </p>
        </div>
      ...
      <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div className="text-sm text-slate-500 dark:text-slate-400">
          Total for {formatMonth(editMonthISO)}:{' '}
          <span className="font-semibold text-slate-900 dark:text-slate-100">
      ...
      <div className="space-y-3">
        {plan.expenses.map((it) => {
          ...
          const hasOverride = (it.overrides || []).some((x) => x.monthISO === editMonthISO);
          const hasChange = (it.changes || []).some((x) => x.monthISO === editMonthISO);
            ...
            >
              ...
              <div className="md:col-span-12 mt-1 flex flex-wrap items-center justify-between gap-2">
                <div className="text-xs text-slate-500 dark:text-slate-400">
                  {hasChange && it.behavior === 'carryForward' && (
                    <span className="mr-2 rounded-full bg-slate-100 px-2 py-1 dark:bg-slate-900">Has change</span>
                  )}
                  {hasOverride && it.behavior === 'monthOnly' && (
                    <span className="mr-2 rounded-full bg-slate-100 px-2 py-1 dark:bg-slate-900">Has override</span>
                  )}
                </div>
      ...
      <div className="mt-10">
        <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">One-time expenses</h2>
