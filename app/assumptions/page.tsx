...
export default function AssumptionsPage(){
  ...
  const lbl='text-xs font-medium text-slate-900 dark:text-slate-100';
    ...
    <div className='space-y-6'>
      ...
      <div className='rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900'>
        <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
          <label><div className={lbl}>Currency</div><input className={inp} value={plan.currency} onChange={e=>update({currency:e.target.value})} placeholder='USD'/></label>
          <label><div className={lbl}>Start month (YYYY-MM)</div><input className={inp} value={plan.startMonthISO} onChange={e=>update({startMonthISO:e.target.value})} placeholder='2026-01'/></label>
          <label><div className={lbl}>Goal net worth ({cur})</div><input className={inp} value={goalDraft} onChange={e=>setGoalDraft(e.target.value)} onBlur={()=>{const v=parseNum(goalDraft);update({goalNetWorth:v});setGoalDraft(new Intl.NumberFormat('en-US',{style:'currency',currency:cur,maximumFractionDigits:0}).format(v));}}/></label>
          <label><div className={lbl}>Expected return (annual)</div><div className='mt-1 flex items-center rounded-xl border border-slate-200 bg-white shadow-sm focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-200 dark:border-slate-800 dark:bg-slate-900'><input className='w-full bg-transparent px-3 py-2 text-sm text-slate-900 outline-none dark:text-slate-100' value={retDraft} onChange={e=>setRetDraft(e.target.value)} onBlur={()=>{const v=parseNum(retDraft);update({expectedReturnPct:v});setRetDraft(v+'%');}}/><span className='pr-3 text-sm text-slate-500'>%</span></div></label>
        </div>
        <div className='mt-5 grid gap-4 md:grid-cols-2'>
          <div>
            <div className={lbl}>Net worth mode</div>
            ...
            <div className='mt-2 text-xs text-slate-500 dark:text-slate-100'>{modeHelp}</div>
            <div className='mt-1 text-xs text-slate-400 dark:text-slate-100'>Tip: Most people should use Reality-anchored projection.</div>
          </div>
      ...
      <div className='rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900'>
        ...
        <div className='mt-1 text-sm text-slate-500'>Manage accounts on the <span className='font-medium text-slate-700 dark:text-slate-100'>Net Worth</span> page.</div>
      </div>
    </div>
  );
}